...
`core-assets.lock` pins the four Linux and macOS `core-v0.1.4` payloads used by the standard
...
Before publishing a new core, exercise the final bundled extension with real authenticated clients:
...
recovery, all four Flight SQL key-metadata schemas with populated and empty results, and GizmoSQL
ADBC prepared `SET` and TPC-H Q20 results/schema handling.
